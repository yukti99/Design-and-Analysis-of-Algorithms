...
/*
int m[100][100],p=0,q=0;
int ts[100],c=0;
void AllTopSorts(struct Graph* g,int d[],int ts[]){
	int flag = 0,i,v,k,size=0;
    	for(i=0;i<g->V;i++){ 
    	
    		q=0;
	    	//find a vertex i which has no incoming edge   
	    	if (!g->visited[i] && g->indegree[i]==0){
	       		struct Node* n = g->adjlists[i];
	       		//indegree(g);
	       		// decrease the indegree of all the adjacent vertices of i
	       		while(n){
			    v = n->vertex;
			    g->indegree[v]--;
			    n = n->next;            
	       		...
	       		//printf("i = %d\n",i);
            		...
            		// reseting values after returning
            		g->visited[i] = 0;
            		n = g->adjlists[i];
            		//size = sizeof(ts)/sizeof(ts[0]);
            		ts[g->V-1] = -1;
            		while(n){
			    v = n->vertex;
			    g->indegree[v] = d[v];
